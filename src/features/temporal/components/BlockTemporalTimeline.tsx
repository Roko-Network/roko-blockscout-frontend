...
const TimelineSvg: React.FC<TimelineProps> = ({ entries, blockNumber }) => {
  ...
  return (
    ...
    >
      { /* Axis line */ }
      <line x1={ 0 } y1={ AXIS_Y } x2={ CHART_W } y2={ AXIS_Y } stroke="var(--chakra-colors-border-divider)" strokeWidth="1.5"/>
      ...
      { positions.map((pos, i) => {
        ...
        const fill = inherent ? 'var(--chakra-colors-text-secondary)' : 'var(--chakra-colors-link-primary)';
        ...
        return (
          ...
          >
            ...
            <text
              ...
              textAnchor="middle"
              fontSize="8"
              fill={ inherent ? 'var(--chakra-colors-text-secondary)' : 'var(--chakra-colors-text-primary)' }
              fontFamily="monospace"
            >
              { label }
            </text>
      ...
      { stamped.length === 1 && positions[0] && (
        <text
          ...
          textAnchor="middle"
          fontSize="9"
          fill="var(--chakra-colors-text-secondary)"
          fontFamily="monospace"
        >
          single tx
        </text>
          ...
          { data && data.length === 0 ? (
            ...
            >
              ...
              <Flex gap={ 4 } mt={ 1 } justifyContent="flex-end">
                { ([ [ 'green.500', '< 1 ms' ], [ 'yellow.500', '1\u2013100 ms' ], [ 'red.500', '\u2265 100 ms' ] ] as const).map(([ color, label ]) => (
