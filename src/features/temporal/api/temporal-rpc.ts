...
import type { TemporalWatermark, TemporalConsensusTime, TemporalQueueStats } from 'src/features/temporal/types/api';

const RPC_URL = config.chain.rpcUrls[0];
...
async function rpcCall<T>(method: string, params: Array<unknown> = []): Promise<T> {
  if (!RPC_URL) {
    throw new Error('ROKO RPC URL is not configured');
  }
